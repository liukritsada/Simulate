    ...
    <style>
        * {
        ...
        :root {
            ...
            --glass-shadow: 0 8px 32px 0 rgba(255, 255, 255, 0.2);
            --text: #212529;
            --text-light: #495057;
        }
        body {
        ...
        .btn-success { background: rgba(100,200,100,0.3); }
        .btn-danger { background: rgba(255,100,100,0.3); }
        .btn-warning { background: rgba(255,200,100,0.3); }
        .nav-tabs {
            display: flex;
            background: var(--glass);
            backdrop-filter: blur(12px);
        ...
        .nav-tab.active { background: var(--glass-light); border-bottom: 3px solid rgba(255, 255, 255, 0.8); opacity: 1; }
        .main-content {
            background: var(--glass);
            backdrop-filter: blur(12px);
            border: 1px solid var(--glass-border);
        ...
        .stations-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; }
        .floor-title { font-size: 1.8em; font-weight: bold; }
        .stations-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 20px; }
        .station-icon-card {
        ...
        .station-icon-card:hover { transform: translateY(-5px); box-shadow: 0 12px 35px rgba(255, 255, 255, 0.4); }
        .station-icon { font-size: 2.2em; margin-bottom: 10px; }
        .station-icon-name { font-weight: bold; margin-bottom: 5px; }
        .station-icon-code {
        ...
        .modal {
            ...
            z-index: 1000; /* Base Z-index for main modals (e.g., Station Detail) */
        ...
        .wizard-step h3 { margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid var(--glass-border); }
        .tab-btn {
            background: none;
            border: none;
            padding: 12px 20px;
        ...
        .row-header { display: grid; grid-template-columns: 2fr 1.5fr auto; gap: 10px; margin-bottom: 10px; align-items: end; }
        .row-details { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 10px; margin-top: 10px; font-size: 12px; }
        .room-card {
            background: rgba(255, 255, 255, 0.5);
            padding: 15px;
            border-radius: 10px;
        ...
        .room-header { font-weight: bold; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; }
        .equipment-list { margin-bottom: 15px; }
        .equipment-item {
            display: grid;
        ...
        .procedure-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 10px; }
        .procedure-checkbox {
            display: flex;
            align-items: center;
            gap: 8px;
        ...
        .procedure-checkbox input { cursor: pointer; }
        ...
        .progress-bar {
            background: linear-gradient(90deg, #1E8449, #1E8449);
            height: 100%;
            width: 25%;
            ...
            <div id="allfloors" class="tab-content active">
                <div class="stations-container">
                    <div class="stations-header">
                        <div class="floor-title">📊 สถานีทั้งหมด</div>
                    </div>
                    <div id="allfloors-stats" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 25px;"></div>
                    <div id="allfloors-list"></div>
                </div>
            </div>
                    ...
                    <div id="simpleStationFields" style="display: none;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px;">
                            <div class="form-group">
                                ...
                                <input type="number" id="defaultWaitTime" class="form-control" min="0" max="480" value="10" placeholder="เช่น 10">
                                <small style="font-size: 11px; color: var(--text-light);">เวลารอคาดการณ์</small>
                            </div>
                            <div class="form-group">
                                <label class="form-label">เวลาทำงาน (นาที) *</label>
                                <input type="number" id="defaultServiceTime" class="form-control" min="1" max="480" value="5" placeholder="เช่น 5">
                                <small style="font-size: 11px; color: var(--text-light);">เวลาให้บริการ</small>
                            </div>
                            <div class="form-group">
                                <label class="form-label">จำนวนพนักงาน *</label>
                                <input type="number" id="staffCount" class="form-control" min="1" max="50" value="2" placeholder="เช่น 2">
...
<div id="room-tab-2" class="wizard-step" style="display:none;">
    <h4 style="margin-bottom: 15px;">หัตถการที่ห้องนี้สามารถทำได้</h4>
    
    <div id="roomProceduresContainer">
        <div id="allProceduresCheckboxContainer">
            <!-- allProcedures checkbox และปุ่มจะถูกสร้างโดย JavaScript -->
        </div>
...
function switchTab(tabName) {
    ...
    if (tabName === 'allfloors') {
        ...
        function switchWizardTab(tab) {
            ...
            document.getElementById(`wizard-tab-${tab}`).style.display = 'block';
        ...
        function validateWizardTab(tab) {
            switch (tab) {
                case 1:
                    ...
                    if (stationType.value === 'simple') {
                        // Validate simple station fields
                        const waitTime = parseInt(document.getElementById('defaultWaitTime').value);
                        const serviceTime = parseInt(document.getElementById('defaultServiceTime').value);
                        const staffCount = parseInt(document.getElementById('staffCount').value);
                        if (isNaN(waitTime) || waitTime < 0) {
                            alert('❌ กรุณากรอกเวลารอที่ถูกต้อง');
                            return false;
                        }
                        if (isNaN(serviceTime) || serviceTime < 1) {
                            alert('❌ กรุณากรอกเวลาทำงานที่ถูกต้อง (อย่างน้อย 1 นาที)');
                            return false;
                        }
                        if (isNaN(staffCount) || staffCount < 1) {
                            alert('❌ กรุณากรอกจำนวนพนักงานที่ถูกต้อง (อย่างน้อย 1 คน)');
                            return false;
                        }
                        
                        // Validate staff schedules
                        const schedules = collectStaffSchedules();
                        if (schedules.length === 0) {
                            alert('❌ กรุณากำหนดตารางเวลาทำงานของพนักงานอย่างน้อย 1 ชุด');
                            return false;
                        }
                        ...
                        schedules.forEach((s, index) => {
                            if (s.start >= s.breakStart || s.breakStart >= s.breakEnd || s.breakEnd >= s.end) {
                                alert(`❌ ตารางเวลาทำงานชุดที่ ${index + 1} ไม่ถูกต้อง: ต้องเป็น (เข้างาน < พักเริ่ม < พักสิ้นสุด < เลิกงาน)`);
                                validSchedules = false;
                            }
                        });
                        if (!validSchedules) return false;

                    } else if (stationType.value === 'with_rooms') {
                    ...
                    return true;
                case 4:
                    if (parseInt(document.getElementById('roomCount').value) < 1) {
        ...
        function openWizard(floor) {
            ...
            wizardData.rooms = {};
            document.getElementById('createStationWizard').style.display = 'block';
            currentWizardTab = 1;
            updateWizardDisplay();
            loadDepartments();
        }
        ...
        function closeWizard() {
            document.getElementById('createStationWizard').style.display = 'none';
            wizardData = {
                station_name: '',
                station_type: '',
                floor: '',
            ...
            updateStaffScheduleUI(); // Reset UI on close
        }
        ...
        function onStationTypeChange() {
            ...
            if (stationType.value === 'simple') {
                // Initialize staff schedules if empty
                if (wizardData.staff_schedules.length === 0) {
	                    // Add default schedule for 2 staff members
	                    for (let i = 0; i < wizardData.staff_count; i++) {
	                        wizardData.staff_schedules.push({
                                name: `พนักงาน ${i + 1}`,
                                count: 1,
	                            start: '08:00',
            ...
            document.querySelectorAll('.station-type-option').forEach(opt => {
                opt.style.border = '2px solid transparent';
            });
            const selectedOption = document.querySelector(`.station-type-option[data-type="${stationType.value}"]`);
            if (selectedOption) {
                selectedOption.style.border = '2px solid #0047AB';
            }
            
            if (stationType.value === 'simple') {
                // Show simple station fields
                simpleFields.style.display = 'block';
                withRoomsFields.style.display = 'none';
                
                // Hide other tabs for simple station
                tabBtn2.style.display = 'none';
                tabBtn3.style.display = 'none';
                tabBtn4.style.display = 'none';
                
                // Update wizard to only have 1 tab
                window.totalWizardTabs = 1;
                updateWizardDisplay();
                
            } else if (stationType.value === 'with_rooms') {
                // Show with_rooms station fields
                simpleFields.style.display = 'none';
                withRoomsFields.style.display = 'block';
                
                // Show all tabs for with_rooms station
                tabBtn2.style.display = 'block';
                tabBtn3.style.display = 'block';
                tabBtn4.style.display = 'block';
                
                // Update wizard to have 4 tabs
    ...
    closeRoomDetailsModal();
        ...
        async function submitWizard(event) {
            ...
            if (wizardData.station_type === 'simple') {
                ...
                stationData.staff_schedules = wizardData.staff_schedules; // ADDED: Required for simple station validation
                stationData.department_id = null;
                stationData.room_count = 0;
            } else if (wizardData.station_type === 'with_rooms') {
                // With rooms station data (original format)
                const room_settings = {};
            ...
            console.log('Submitting:', stationData);

            try {
                const response = await fetch(`${API_BASE_URL}/create_station.php`, {
                    method: 'POST',
                ...
                console.log('Response:', result);
                  ...
                  if (result.success) {
    const createdFloor = stationData.floor; // Save floor before closing wizard
    alert(`✅ สร้างสถานีแพทย์สำเร็จ!\n\nรหัส: ${result.data.station_code}`);
    closeWizard();
    
    // ✅ FIX: อัปเดตทั้ง All Floors และ Floor เดี่ยว
    loadAllFloorsEnhanced();  // ← เพิ่มบรรทัดนี้
...
function createStationCard(station, stats, stationType) {
    if (station.station_id === '83') {
        stats = {
            total_patients: 10,
            in_progress_patients: 2,
            pending_patients: 8
        };
    }
    ...
    const isSpecialStation = station.station_id === '83';
    const cardClass = `station-card-compact ${isSpecialStation ? 'special-station' : ''}`;

    return `
        <div class="${cardClass}" onclick="openStationDetail(${station.station_id})">
            <button 
...
function createFallbackCard(station) {
    const isSpecialStation = station.station_id === '83';
    const cardClass = `station-card-compact loading ${isSpecialStation ? 'special-station' : ''}`;
    ...
    try {
        ...
        async function submitImport() {
	        ...
	        async function loadPatients() {
	            ...
	            try {
	                const response = await fetch(`${API_BASE_URL}/get_patients.php?action=list&date=${date}&status=${status}&doctor=${doctor}&station=${station}`);
                const result = await response.json();
...
// loadAllFloorsEnhanced();
    ...
    <script src="js/modules/11-auto-assignment-system.js"></script>
