<?php
/**
 * ✅ DELETE Station Procedure API
 * POST /api/delete_station_procedure.php
 * 
 * Request:
 * {
 *   "procedure_id": 2518
...
try {
    ...
    // ✅ Get parameters
    $procedure_id = isset($input['procedure_id']) ? (int)$input['procedure_id'] : null;

    if (!$procedure_id) {
        throw new Exception('procedure_id is required');
    }
    ...
    $query = "
        SELECT procedure_id, station_id, procedure_name
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Procedure deleted successfully from station',
        'data' => [
            'procedure_id' => $procedure_id,
            'station_id' => $station_id,
            'procedure_name' => $procedure_name
